...
export const getAdvertisements = async (req, res) => {
  ...
  try {
    ...
    const advertisements = result.rows.map((row) => ({
      ...
      photo_url: row.photo_url || DEFAULT_IMAGES.ADVERTISEMENT,
...
export const createAdvertisement = async (req, res) => {
  try {
    ...
    const defaultPhotoUrl = DEFAULT_IMAGES.ADVERTISEMENT;
    ...
    const result = await connection.query(
      queries.advertisements.createAdvertisement,
      [
        title,
        description,
        category,
    ...
    res
      .status(500)
      .json({ message: "Error al crear la publicidad", error: error.message });
  }
};
...
export const updateAdvertisement = async (req, res) => {
  let connection;
  const { id } = req.params;
  const { title, description, category, photo_url, related_product_id } =
    req.body;
  ...
  try {
    ...
    const result = await connection.query(
      queries.advertisements.updateAdvertisement,
      [
        title,
        description,
        category,
...
export const deleteAdvertisement = async (req, res) => {
  ...
  if (!id) {
    return res
      .status(400)
      .json({ message: "El ID de la publicidad es requerido" });
  }

  try {
    connection = await getConnection();
    const result = await connection.query(
      queries.advertisements.deleteAdvertisement,
      [id]
    );
    ...
    if (result.rowCount === 0) {
      return res
        .status(404)
        .json({ message: "Publicidad no encontrada o ya eliminada" });
    }
