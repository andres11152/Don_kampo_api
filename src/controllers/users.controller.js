...
export const getUsers = async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ msg: "Error al obtener los usuarios", error: error.message });
  } finally {
...
export const getUsersById = async (req, res) => {
  ...
  try {
    ...
    const ordersResult = await client.query(queries.users.getUserOrdersById, [
      id,
    ]);
...
export const createUsers = async (req, res) => {
  // LÓGICA CORRECTA PARA CREAR UN USUARIO
  const {
    user_name,
    lastname,
    email,
  ...
  try {
    ...
    // Verificar si el email ya existe usando query centralizado
    const emailCheck = await client.query(queries.users.checkEmailExists, [
      email,
    ]);
    ...
    await client.query(queries.users.createUsers, [
      user_name,
      safeLastname,
      email,
      phone,
...
export const updateUserStatus = async (req, res) => {
  ...
  if (!id) {
    return res
      .status(400)
      .json({ msg: "Por favor proporciona un ID de usuario." });
  }
  ...
  try {
    ...
    const result = await client.query(queries.users.updateUserStatus, [
      newStatus,
      id,
    ]);
    ...
    res
      .status(200)
      .json({ msg: "Estado del usuario actualizado exitosamente." });
  } catch (error) {
...
export const changePassword = async (req, res) => {
  ...
  if (!currentPassword || !newPassword) {
    return res
      .status(400)
      .json({ msg: "La contraseña actual y la nueva son requeridas." });
  }
  ...
  try {
    ...
    // 1. Obtener el hash de la contraseña actual del usuario usando query centralizado
    const userResult = await client.query(queries.users.getUserPasswordById, [
      userId,
    ]);
    ...
    // 3. Hashear y actualizar la nueva contraseña usando query centralizado
