...
export const placeOrder = async (req, res) => {
  ...
  try {
    ...
    const productCheckResult = await client.query(
      `SELECT product_id FROM products WHERE product_id = ANY($1)`, [productIds]
    );
    ...
    if (invalidProducts.length > 0) {
      return res.status(400).json({ msg: 'Productos inexistentes.', invalidProducts });
    }
    ...
    if (userId === 'guest-user') {
      accessToken = jwt.sign({ orderId: orderId, guest: true }, authConfig.secret, { expiresIn: '1h' });
    }
...
export const getOrders = async (req, res) => {
  ...
  try {
    ...
    const userDataResult = await client.query(`SELECT order_id, user_data FROM user_data WHERE order_id = ANY($1)`, [orderIds]);
    const userDataMap = userDataResult.rows.reduce((acc, row) => ({...acc, [row.order_id]: row.user_data}), {});
...
export const getOrdersById = async (req, res) => {
  ...
  try {
    ...
    if (orderResult.rows.length === 0) return res.status(404).json({ msg: 'No encontrado.' });
    ...
    const userDataResult = await client.query(`SELECT user_data FROM user_data WHERE order_id = $1`, [orderId]);
...
export const updateOrderStatus = async (req, res) => { 
  ...
  } catch (e) { res.status(500).json({ msg: 'Error.' }); } finally { if(client) client.release(); }
};
...
export const deleteOrders = async (req, res) => {
  ...
  try {
    ...
    const check = await client.query('SELECT id FROM orders WHERE id = $1', [id]);
    if(check.rowCount === 0) return res.status(404).json({ msg: 'No encontrado' });
...
export const updateBulkOrders = async (req, res) => {
  ...
  if (!orderIds || !Array.isArray(orderIds)) return res.status(400).json({ msg: 'Faltan IDs' });
  ...
  try {
    ...
    const updateResult = await client.query(queries.orders.updateBulkOrderStatus, [newStatus, numericIds]);
    await client.query("COMMIT");
    res.status(200).json({ success: true, updatedCount: updateResult.rowCount });
  } catch (e) {
...
export const updateOrderById = async (req, res) => {
  const { orderId } = req.params;
  let { status_id, requires_electronic_billing, company_name, nit, user_type, shipping, userData, items, itemsToRemove, shipping_cost, shipping_percentage } = req.body;
  ...
  try {
    ...
    const orderLock = await client.query(`SELECT id, customer_id FROM orders WHERE id = $1 FOR UPDATE`, [orderId]);
    ...
    if (status_id !== undefined) { fieldsToUpdate.push(`status_id = $${idx++}`); values.push(status_id); }
    if (requires_electronic_billing !== undefined) { fieldsToUpdate.push(`requires_electronic_billing = $${idx++}`); values.push(requires_electronic_billing); }
    if (company_name !== undefined) { fieldsToUpdate.push(`company_name = $${idx++}`); values.push(company_name); }
    if (nit !== undefined) { fieldsToUpdate.push(`nit = $${idx++}`); values.push(nit); }
    if (user_type !== undefined) { fieldsToUpdate.push(`user_type = $${idx++}`); values.push(user_type); }

    if (fieldsToUpdate.length > 0) {
      values.push(orderId);
      await client.query(`UPDATE orders SET ${fieldsToUpdate.join(', ')} WHERE id = $${idx}`, values);
    }
    ...
    if (Array.isArray(items) && items.length > 0) {
      ...
      for (const it of normalizedItems) {
        const existing = await client.query(
          `SELECT id FROM order_items WHERE order_id = $1 AND product_id = $2 AND variation_id = $3 AND (presentation_id = $4 OR (presentation_id IS NULL AND $4 IS NULL)) LIMIT 1`,
          [orderId, it.productId, it.variationId, it.presentationId || null]
        );

        if (existing.rowCount > 0) {
          await client.query(
            `UPDATE order_items SET price = $1, quantity = $2, quality = $3, presentation = $4 WHERE id = $5`,
    ...
    if (Array.isArray(itemsToRemove) && itemsToRemove.length > 0) {
        for (const r of itemsToRemove) {
            await client.query(
                `DELETE FROM order_items WHERE order_id = $1 AND product_id = $2 AND variation_id = $3 AND (presentation_id = $4 OR (presentation_id IS NULL AND $4 IS NULL))`,
                [orderId, r.productId ?? r.product_id, r.variationId ?? r.variation_id, r.presentationId ?? r.presentation_id ?? null]
            );
        }
    }
    ...
    if (shipping) {
      ...
      if (sh.rowCount > 0) {
        await client.query(
          ...
          [shipping.shippingMethod, shipping.trackingNumber, shipping.estimatedDelivery, shipping.actualDelivery, shipping.shippingStatusId, orderId]
        );
      } else {
        await client.query(
      ...
      if (shipping.shipping_cost !== undefined) shipping_cost = Number(shipping.shipping_cost);
    }

    const totalRes = await client.query(`SELECT COALESCE(SUM(price * quantity), 0) as sub FROM order_items WHERE order_id = $1`, [orderId]);
    const sub = Number(totalRes.rows[0].sub);
    const newShipping = shipping_cost ? Number(shipping_cost) : 0;
    
    await client.query(`UPDATE orders SET total = $1, shipping_cost = $2 WHERE id = $3`, [sub + newShipping, newShipping, orderId]);
...
export const updateOrderPrices = async (req, res) => {
  ...
  try {
    ...
    console.log("--- INICIANDO ACTUALIZACIÓN (QUERY CORREGIDO PARA TABLA 'product_presentations') ---");
    ...
    const uniqueProductIds = [...new Set(itemsToUpdate.map(i => i.product_id))];
    ...
    for (const row of productsResult.rows) {
      ...
      if (row.presentations && Array.isArray(row.presentations)) {
        ...
        const validPresentations = row.presentations.filter(p => p.presentation);
    ...
    for (const item of itemsToUpdate) {
      let safeUserType = item.user_type ? String(item.user_type).toLowerCase().trim() : 'hogar';
      ...
      if (newPrices) {
         ...
         if (precioNuevo !== undefined && precioNuevo !== precioViejo) {
            console.log(`✅ ACTUALIZANDO Item ${item.item_id}: ${item.presentation} => ${precioNuevo}`);
            updatePromises.push(
                client.query(
                  `UPDATE order_items SET price = $1 WHERE id = $2`,
                  [precioNuevo, item.item_id]
                )
            );
            updatesCount++;
         }
         ...
         console.warn(`❌ NO MATCH: Prod ${item.product_id}. Busqué: "${itemPresName}". DB: [${available.join(', ')}]`);
      }
    }
