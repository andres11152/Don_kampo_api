...
export const placeOrder = async (req, res) => {
  ...
  try {
    ...
    const aggregatedCart = cartDetails.reduce((acc, item) => {
      ...
      // Esto evita que se mezclen datos de diferentes calidades con el mismo nombre de presentación
      const key = item.presentation_id
        ? String(item.presentation_id)
        : `${item.variationId}-${item.presentation}`;
    ...
    // Logging de diagnóstico para validar agregación
    console.log(
      `📦 Carrito agregado: ${Object.keys(aggregatedCart).length} items únicos`
    );
    for (const [key, item] of Object.entries(aggregatedCart)) {
      if (!item.presentation_id) {
        console.warn(`⚠️ Item sin presentation_id:`, {
          key,
...
export const updateOrderPrices = async (req, res) => {
  ...
  try {
    ...
    console.log(`\n🔍 DIAGNÓSTICO INICIAL:`);
    console.log(
      `   Órdenes pendientes encontradas: ${itemsToUpdate.length} items`
    );

    if (itemsToUpdate.length === 0) {
      await client.query("COMMIT");
      console.log("⚠️ No hay órdenes en estado PENDIENTE (status_id=1)");
      return res.status(200).json({ msg: "No hay ítems pendientes." });
    }

    console.log(
      `   Productos únicos: ${
        [...new Set(itemsToUpdate.map((i) => i.product_id))].length
      }`
    );
    console.log(`\n📝 ITEMS A PROCESAR:`);
    itemsToUpdate.forEach((item) => {
      console.log(
        `   - Item ${item.item_id}: Prod ${item.product_id}, Var ${item.variation_id}, Pres "${item.presentation}", PrecioActual: $${item.old_price}`
      );
    });
    ...
    // 3. Crear Mapas con DOBLE INDEXACIÓN para matching robusto
    ...
    for (const item of itemsToUpdate) {
      ...
      // NIVEL 1: Match STRICT por variación + nombre normalizado
      if (!newPrices) {
      ...
      // NIVEL 2: Match FUZZY (elimina TODO excepto letras y números)
      if (!newPrices) {
      ...
      // NIVEL 3: FALLBACK si solo hay una presentación disponible
      if (!newPrices) {
      ...
      if (newPrices) {
        ...
        if (precioNuevo !== undefined && precioNuevo !== precioViejo) {
          ...
          updatesCount++;
        } else if (precioNuevo !== undefined) {
          console.log(
            `⏭️ SKIP - Item ${item.item_id}: "${item.presentation}" ya tiene el precio correcto ($${precioViejo})`
          );
        }
      } else {
