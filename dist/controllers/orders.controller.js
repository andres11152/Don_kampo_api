...
export const placeOrder = async (req, res) => {
  ...
  try {
    ...
    const orderResult = await client.query(queries.orders.createOrder, [userId, new Date(), 1,
    // status_id (por ejemplo, 1 = pendiente, 2 = enviado, etc.)
    total]);
    ...
    if (shippingMethod && estimatedDelivery && actual_delivery) {
      ...
      await client.query(queries.shipping_info.createShippingInfo, [shippingMethod, trackingNumber, estimatedDelivery, actual_delivery, shippingStatusId,
      // Estado de envío (por ejemplo, 1 = pendiente, 2 = enviado, etc.)
      orderId]);
    }
    client.release();
