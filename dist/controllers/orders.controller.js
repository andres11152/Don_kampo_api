...
export const placeOrder = /*#__PURE__*/function () {
  var _ref = _asyncToGenerator(function* (req, res) {
    ...
    try {
      ...
      // Crear la orden
      const orderResult = yield client.query(queries.orders.createOrder, [userId, new Date(), 1, total, needsElectronicInvoice, companyName || null, companyNit || null, user_type]);
      ...
      for (const item of aggregatedItems) {
        yield client.query(queries.orders.createOrderItem, [orderId, item.productId, item.quantity,
        // Cantidad total agrupada
        item.price, item.variationId, item.quality, item.presentation, item.presentation_id]);
      }
...
export const getOrdersById = /*#__PURE__*/function () {
  var _ref3 = _asyncToGenerator(function* (req, res) {
    try {
      ...
      const variationIds = orderItems.map(item => item.variation_id);
      const variationsResult = yield client.query(`SELECT variation_id, product_id, quality, presentations
          FROM product_variations
  ...
  var _ref8 = _asyncToGenerator(function* (req, res) {
    ...
    `);
      ...
      for (const pid of productIds) {
        ...
        const variationsResult = yield client.query(queries.products.getProductVariations, [pid]);
      ...
      for (const item of orderItems) {
        const userType = ordersResult.rows.find(({
          id
        }) => id === item.order_id).user_type;
        const variation = variationsMap[item.product_id].find(({
          variation_id
        }) => variation_id === item.variation_id);
        if (variation) {
          const presentation = variation.presentations.find(({
            presentation_id
