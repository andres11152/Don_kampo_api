    ...
    try {
      ...
      if (!email) {
        return res.status(400).json({
          msg: 'Por favor, ingresa un correo electrónico.'
        });
      }
      ...
      const result = yield client.query(queries.users.getUserByEmail, [normalizedEmail]);
      ...
      yield sendEmail(email, 'Código para restablecer tu contraseña', `Tu código de restablecimiento de contraseña es: ${verificationCode}`, provider);
      res.status(200).json({
  ...
  var _ref2 = _asyncToGenerator(function* (req, res) {
    ...
    try {
      ...
      if (!email || !code || !newPassword) {
        return res.status(400).json({
          msg: 'Correo, código y nueva contraseña son requeridos.'
        });
      }
      ...
      const result = yield client.query(queries.users.verifyUserResetCode, [email.toLowerCase(),
      // Normalizar el correo a minúsculas
      code, currentTimeInSeconds]);
      if (result.rows.length === 0) {
        return res.status(400).json({
